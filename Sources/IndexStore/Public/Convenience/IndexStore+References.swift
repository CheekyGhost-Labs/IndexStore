...
public extension IndexStore {
    ...
    /// Will return propery declaration symbol occurences of the resolved type of the given symbol.
    ///
    ...
    func sourceSymbols(forPropertiesWithTypeOf symbol: SourceSymbol) -> [SourceSymbol] {
        ...
        // Resolve valid occurences based on roles
        let rawResults = workspace.occurrences(ofUSR: symbol.usr, roles: [.reference, .containedBy])
        let baseOccurences = rawResults.map(sourceSymbolFromOccurence)
        // Grab the property parents of valid occurences
    ...
    /// Will return propery declaration symbol occurences of the resolved type of the given symbol within the given source files.
    ///
    /// for example, in the following source
    /// ```
    /// class MyClass {}
    ...
    /// Will return constructor/initializer symbol occurences of the given symbol.
    ///
    /// for example, in the following source
    /// ```
    /// class MyClass {}
    ...
    /// ```
    /// let classSymbol = indexStore.querySymbols(.classDeclarations(matching: "CustomClass"))[0]
    /// indexStore.sourceSymbols(initializing: classSymbol) // [myInstance = MyClass()]
    /// ```
    /// - Parameter symbol: The symbol to search with.
    /// - Returns: `Array` of ``SourceSymbol`` objects.
    func sourceSymbols(initializing symbol: SourceSymbol) -> [SourceSymbol] {
        queryRelatedOccurences(
            ofSymbol: symbol,
            query: .withRoles([.reference, .call, .calledBy, .containedBy]).withKinds(SourceKind.properties)
        )
    }
    
    /// Will return constructor/initializer symbol occurences of the given symbol within the given array of source file paths.
    ///
    /// for example, in the following source
    /// ```
    /// class MyClass {}
    ...
    /// ```
    /// let classSymbol = indexStore.querySymbols(.classDeclarations(matching: "CustomClass"))[0]
    /// indexStore.sourceSymbols(initializing: classSymbol, in: ["source", "file", "paths"]) // [myInstance = MyClass()]
    /// ```
    /// - Parameters:
    ///   - symbol: The symbol to search with.
    ///   - sourceFiles: Array of source files to search in.
    /// - Returns: `Array` of ``SourceSymbol`` objects.
    func sourceSymbols(initializing symbol: SourceSymbol, in sourceFiles: [String]) -> [SourceSymbol] {
        let query = IndexStoreQuery.withRoles([.reference, .call, .calledBy, .containedBy])
            .withKinds(SourceKind.properties)
            .withSourceFiles(sourceFiles)
        return queryRelatedOccurences(ofSymbol: symbol, query: query)
    }
}
